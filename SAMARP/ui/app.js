...
class PortalController {
  constructor() {
    ...
    // Connect to SAMARP FastAPI backend (non-blocking — graceful fallback)
  ...
  initEventListeners() {
    ...
    // Backend Export Buttons (real signed hash-chain data)
  ...
  initSOCFeatures() {
    ...
    // 5. Populate Initial Incident Feed (mock seed; backend overwrites on connect)
    ...
    // 7. Listen for live incidents from BackendBridge
  ...
  initIncidentFeed() {
    ...
    // Seed initial incidents from mock database as placeholders
    ...
    // Pre-populate with real backend incidents if available
    if (window.BackendBridge) {
      window.BackendBridge.fetchIncidents({ limit: 15 }).then(incidents => {
        if (!incidents || incidents.length === 0) return;
        // Clear mock seeds, replace with real data
        feed.innerHTML = '';
        incidents.forEach(inc => feed.appendChild(this.createIncidentDOM(inc)));
        const badge = document.getElementById('incidentCountBadge');
        if (badge) badge.textContent = `${incidents.length} INCIDENTS`;
      });
    }
  }
